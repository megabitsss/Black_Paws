...
func _ready():
	...
	match last_movement:
		...
		Vector2(1,1), Vector2(-1, -1), Vector2(-1, 1), Vector2(1, -1):
			...
			move_to_more = global_position + Vector2(last_movement.x * randf_range(0, 0.25), last_movement.y)*500
	...
	initial_tween.play() #to make sure that it's not stopped
	...
	if set_angle == 1:
		angle = angle_less
		tween.tween_property(self, 	"angle", angle_more, 2) #in 2 secs
